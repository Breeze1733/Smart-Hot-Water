        ...
        Page({
            ...
            switchWater: function(isOpen, macNumber) {
                ...
                var callback = function(success, resultData) {
                    ...
                    if (typeof window !== "undefined" && window.logToConsole) {
                        if (success) {
                            window.logToConsole("[Callback SUCCESS] " + actionName + " [" + factoryName + "] 链路终点执行成功！响应数据: " + detailStr, "success");
                        } else {
                            window.logToConsole("[Callback ERROR] " + actionName + " [" + factoryName + "] 链路节点抛出错误！错误代码/信息: " + detailStr, "error");
                        }
                    }
                    if (success) {
